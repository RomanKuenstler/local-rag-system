...
function getPendingAssistantMessageText(modeId, chainStage) {
  const mode = String(modeId || "").trim().toLowerCase();
  const stage = String(chainStage || "").trim().toLowerCase();
  if (mode === "refine") {
    if (stage === "refining") {
...
function App() {
  ...
  useEffect(() => {
    if (!isSending) return;
    const nextPendingText = getPendingAssistantMessageText(currentAssistantMode, activeChainStage);
    setMessages((previous) => {
      const pendingIndex = [...previous].reverse().findIndex((message) => message.isPending && message.role === "assistant");
      if (pendingIndex < 0) return previous;
      const absoluteIndex = previous.length - 1 - pendingIndex;
      const pendingMessage = previous[absoluteIndex];
      if (!pendingMessage || pendingMessage.text === nextPendingText) return previous;
  ...
  async function sendRawPrompt(rawPrompt, promptFiles = []) {
    ...
    const pendingText = getPendingAssistantMessageText(currentAssistantMode, activeChainStage);
    setMessages((prev) => prev.concat(createMessage("assistant", pendingText, {
      id: pendingMessageId,
      isPending: true,
    })));
